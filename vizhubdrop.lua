...
-- + Aimbot prediction
-- + Unlimited-range ESP (Highlight + 2D Drawing ESP)
-- ================================================================
...
introScreen.BackgroundColor3 = Color3.fromRGB(10, 10, 14)
introScreen.BorderSizePixel = 0
introScreen.ZIndex = 1000
...
introGradient.Parent = introScreen

local introContainer = Instance.new("Frame")
introContainer.Size = UDim2.new(0, 420, 0, 220)
...
introBarBg.Size = UDim2.new(0, 320, 0, 6)
introBarBg.Position = UDim2.new(0.5, 0, 0, 175)
introBarBg.AnchorPoint = Vector2.new(0.5, 0)
introBarBg.BackgroundColor3 = Color3.fromRGB(30, 30, 40)
introBarBg.BorderSizePixel = 0
introBarBg.ZIndex = 1002
introBarBg.Parent = introContainer
...
introBarFill.BackgroundColor3 = CONFIG.AccentColor
introBarFill.BorderSizePixel = 0
introBarFill.ZIndex = 1003
...
introPercent.Size = UDim2.new(1, 0, 0, 16)
introPercent.Position = UDim2.new(0, 0, 0, 190)
introPercent.BackgroundTransparency = 1
introPercent.Text = "0%"
introPercent.TextColor3 = CONFIG.AccentColor
introPercent.Font = Enum.Font.GothamBold
introPercent.TextSize = 11
introPercent.ZIndex = 1002
introPercent.Parent = introContainer

task.spawn(function()
    while introScreen and introScreen.Parent do
        TweenService:Create(introLogoStroke, TweenInfo.new(1.2, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {
            Transparency = 0.1,
        }):Play()
        task.wait(1.2)
        if not introScreen or not introScreen.Parent then break end
        TweenService:Create(introLogoStroke, TweenInfo.new(1.2, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {
            Transparency = 0.7,
        }):Play()
        task.wait(1.2)
...
introContainer.Position = UDim2.new(0.5, 0, 0.5, 20)
introLogo.TextTransparency = 1
introSub.TextTransparency = 1

TweenService:Create(introContainer, TweenInfo.new(0.7, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
    Position = UDim2.new(0.5, 0, 0.5, 0),
}):Play()

TweenService:Create(introLogo, TweenInfo.new(0.6, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
    TextTransparency = 0,
}):Play()

task.wait(0.3)
TweenService:Create(introSub, TweenInfo.new(0.6), { TextTransparency = 0 }):Play()

TweenService:Create(introLine, TweenInfo.new(0.8, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
    Size = UDim2.new(1, 0, 0, 1),
}):Play()

local introStages = {
    {pct = 20, text = "Loading modules..."},
    {pct = 45, text = "Scanning workspace..."},
    {pct = 65, text = "Initializing bot registry..."},
    {pct = 85, text = "Preparing UI..."},
    {pct = 100, text = "Ready"},
}

for _, stage in ipairs(introStages) do
    TweenService:Create(introBarFill, TweenInfo.new(0.45, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
        Size = UDim2.new(stage.pct / 100, 0, 1, 0),
    }):Play()

    TweenService:Create(introPercent, TweenInfo.new(0.2), {
        TextTransparency = 1,
    }):Play()
    task.wait(0.15)
    introStatus.Text = stage.text
    introPercent.Text = stage.pct .. "%"
    TweenService:Create(introPercent, TweenInfo.new(0.2), {
        TextTransparency = 0,
    }):Play()
    task.wait(0.4)
...
TweenService:Create(introScreen, TweenInfo.new(0.6, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
    BackgroundTransparency = 1,
}):Play()
TweenService:Create(introContainer, TweenInfo.new(0.5), {
...
TweenService:Create(introLine, TweenInfo.new(0.5), { BackgroundTransparency = 1 }):Play()
TweenService:Create(introStatus, TweenInfo.new(0.5), { TextTransparency = 1 }):Play()
TweenService:Create(introBarBg, TweenInfo.new(0.5), { BackgroundTransparency = 1 }):Play()
TweenService:Create(introBarFill, TweenInfo.new(0.5), { BackgroundTransparency = 1 }):Play()
TweenService:Create(introPercent, TweenInfo.new(0.5), { TextTransparency = 1 }):Play()
...
function Menu:RegisterElement(section, name, elemType, elem)
    local secName = section.Name
    Menu.Registry[secName] = Menu.Registry[secName] or {}
    Menu.Registry[secName][name] = {
        Type = elemType,
        Get = function()
            if elem.Get then return elem.Get(elem) end
        end,
        Set = function(v)
            if elem.Set then elem.Set(elem, v) end
        end,
    }
end
...
function Menu:Toggle(section, name, default, callback)
    ...
    clickBtn.MouseButton1Click:Connect(function()
        setVisual(not state, true)
    end)

    local elem = {
        Set = function(_, v)
            if state ~= v then setVisual(v, true) end
        end,
        Get = function() return state end,
    }
    Menu:RegisterElement(section, name, "Toggle", elem)
    return elem
end
...
function Menu:Slider(section, name, min, max, default, callback)
    ...
    local elem = {
        Set = function(_, v) applyValue(v, true) end,
        Get = function() return value end,
    }
    Menu:RegisterElement(section, name, "Slider", elem)
    return elem
end
...
function Menu:ColorPicker(section, name, default, callback)
    ...
    local rFill = makeSlider(40,  "R", r, function(v) r = v end)
    local gFill = makeSlider(75,  "G", g, function(v) g = v end)
    local bFill = makeSlider(110, "B", b, function(v) b = v end)
    ...
    Menu:RegisterElement(section, name, "ColorPicker", elem)
    return elem
end
...
function Menu:Dropdown(section, name, options, default, callback)
    ...
    local elem = {
        ...
        SetOptions = function(_, newOptions, newSelected)
            options = newOptions
    ...
    Menu:RegisterElement(section, name, "Dropdown", elem)
    return elem
end

function Menu:Textbox(section, name, default, callback)
    ...
    box.Position = UDim2.new(1, -128, 0.5, -12)
    ...
    Menu:RegisterElement(section, name, "Textbox", elem)
    return elem
end
...
-- ================================================================
local ConfigSystem = {
    Folder = "VizHub_Configs",
    CurrentConfig = "default",
}

local function ensureConfigFolder()
    if type(isfolder) ~= "function" or type(makefolder) ~= "function" then
        return false, "Executor does not support file operations"
    end
    if not isfolder(ConfigSystem.Folder) then
        local ok, err = pcall(makefolder, ConfigSystem.Folder)
...
local function getConfigPath(name)
    return ConfigSystem.Folder .. "/" .. tostring(name) .. ".json"
end
...
local function saveConfig(name)
    ...
    local ok, err = ensureConfigFolder()
    if not ok then return false, err end
    if type(writefile) ~= "function" then return false, "writefile not available" end

    local data = {}
    for secName, elements in pairs(Menu.Registry) do
        data[secName] = {}
        for elemName, reg in pairs(elements) do
            local success, val = pcall(reg.Get)
            if success then
                data[secName][elemName] = serializeValue(val)
            end
        end
    end

    local encOk, encoded = pcall(function() return HttpService:JSONEncode(data) end)
    if not encOk then return false, "Encode error: " .. tostring(encoded) end

    local wOk, wErr = pcall(writefile, getConfigPath(name), encoded)
...
local function loadConfig(name)
    ...
    if type(isfile) ~= "function" or type(readfile) ~= "function" then
        return false, "File reading not available"
    end
    local path = getConfigPath(name)
    if not isfile(path) then return false, "Config not found" end

    local rOk, content = pcall(readfile, path)
    if not rOk then return false, "Read error: " .. tostring(content) end

    local dOk, data = pcall(function() return HttpService:JSONDecode(content) end)
    if not dOk then return false, "Decode error: " .. tostring(data) end

    for secName, elements in pairs(data) do
        if Menu.Registry[secName] then
            for elemName, val in pairs(elements) do
                local reg = Menu.Registry[secName][elemName]
                if reg then
                    local actualVal = deserializeValue(val)
                    pcall(reg.Set, actualVal)
                end
            end
        end
    end
...
local function deleteConfig(name)
    if type(isfile) ~= "function" or type(delfile) ~= "function" then
        return false, "File operations not available"
    end
    local path = getConfigPath(name)
...
local HealthESP = {
    Enabled = false,
    ColorLow = Color3.fromRGB(255, 60, 60),
    ColorMid = Color3.fromRGB(255, 200, 60),
    ColorHigh = Color3.fromRGB(80, 220, 100),
    ShowHealth = true,
    ShowName = true,
    ShowDistance = false,
    Scale = 1,
...
-- Health ESP — БЕЗ ограничения по дистанции. Рисует всех NPC в BotRegistry.
        ...
        if not model or not model.Parent then
            ...
            if not head or not hum then
                ...
                if hum.Health <= 0 then
                    ...
                    -- БЕЗ лимита дистанции: только проверка на экране
...
-- ==================== ESP (Highlight) ====================
-- Highlight без ограничения дальности: применяется ко всем NPC в BotRegistry.
local ESP = {
    Bots = {
        Enabled = false,
...
local function applyHighlightSettings(h)
    ...
    h.DepthMode = Enum.HighlightDepthMode.AlwaysOnTop   -- сквозь стены, на любой дистанции
end
...
local function getPredictedPosition(part)
    ...
    if AimCursor.Prediction <= 0 then return part.Position end
    local vel = part.AssemblyLinearVelocity
    if not vel then return part.Position end
    if vel.Magnitude < 0.5 then return part.Position end
    return part.Position + vel * AimCursor.Prediction
end
    ...
    for i = #BotRegistry.Bots, 1, -1 do
        ...
        if not model or not model.Parent then
            ...
            if head and head:IsA("BasePart") then
                local predictedPos = getPredictedPosition(head)
...
local aimCursorRender = RunService.RenderStepped:Connect(function()
    ...
    local targetPos = getPredictedPosition(t.head)
    ...
    task.spawn(function()
        while Watermark.Enabled and Watermark.Gui do
            ...
            local text = string.format(
                "VizHub | FPS: %d | Ping: %s | NPCs: %d\nRecoil [LMB]: %s %s\nAimbot [RMB]: %s %s\nTime: %s",
                Watermark.FPS, tostring(ping), #BotRegistry.Bots,
...
local SectionConfig     = Menu:CreateSection("Config")
...
Menu:Slider(SectionAimbot, "Prediction (sec)", 0, 1, 0, function(v)
    AimCursor.Prediction = v
end)
...
-- ==================== CONFIG TAB ====================
Menu:Label(SectionConfig, "── Config Manager ──")

Menu:Textbox(SectionConfig, "Config Name", "default", function(v)
    ConfigSystem.CurrentConfig = v
end)
...
local function refreshConfigList()
    ...
    for _, n in ipairs(list) do table.insert(newOptions, n) end
    if configDropdownElement then
        local sel = ConfigSystem.CurrentConfig
        local found = false
        for _, o in ipairs(newOptions) do
            if o == sel then found = true; break end
        end
        configDropdownElement:SetOptions(newOptions, found and sel or "<none>")
    end
end

configDropdownElement = Menu:Dropdown(SectionConfig, "Saved Configs", {"<none>"}, "<none>",
    function(v)
...
Menu:Button(SectionConfig, "Save Config", function()
    local ok, err = saveConfig(ConfigSystem.CurrentConfig)
    if ok then
        refreshConfigList()
        pcall(function()
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "VizHub",
                Text  = "Saved: " .. ConfigSystem.CurrentConfig,
                Duration = 2,
            })
        end)
    else
...
Menu:Button(SectionConfig, "Load Config", function()
    local ok, err = loadConfig(ConfigSystem.CurrentConfig)
    if ok then
        pcall(function()
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "VizHub",
                Text  = "Loaded: " .. ConfigSystem.CurrentConfig,
                Duration = 2,
            })
        end)
    else
...
Menu:Button(SectionConfig, "Delete Config", function()
    local ok, err = deleteConfig(ConfigSystem.CurrentConfig)
    if ok then
        refreshConfigList()
        pcall(function()
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "VizHub",
                Text  = "Deleted: " .. ConfigSystem.CurrentConfig,
                Duration = 2,
            })
        end)
    else
        warn("[Config] Delete failed:", err)
    end
end)

Menu:Button(SectionConfig, "Refresh List", function()
    refreshConfigList()
end)

Menu:Label(SectionConfig, "Folder: " .. ConfigSystem.Folder)
Menu:Label(SectionConfig, "Save/Load captures ALL menu values.")
